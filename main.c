...
#define VERSION 13 //id for the validation
#define FILTER_TYPE 5// 0=> RSG overworld (Practice Seeds); 1=> village only; 2=> shipwreck only; 3 => jungle only; 4 => coinflip (classic); 5=> whatever I want
#define DEBUG 0
#include <string.h>
#include <time.h>
...
int possible_lava(int64_t lower48, int x, int z){
    ...
    fakeseed = fakeseed ^ 0x5deece66dUL; //population seed set?
...
int lava_biome(int64_t seed, int x, int z, LayerStack* gp){
    ...
    if (lavabiome == plains || lavabiome == desert){
        return 1;
    }
    return 0;
}
...
int valid_structures_and_types(int64_t lower48, int* fortressQuadrant, int filter_style){
    if (filter_style == 5){
        if (possible_lava(lower48, 16, 16) == 0){
            return -100;
        }
        return 1;
    }
