...
uint64_t charTo64bitNum(const char a[]) {
    uint64_t n = (uint64_t) (a[7] & 0xFF);
    n |= (uint64_t) (a[6] & 0xFF) << 8;
    n |= (uint64_t) (a[5] & 0xFF) << 16;
    n |= (uint64_t) (a[4] & 0xFF) << 24;
    n |= (uint64_t) (a[3] & 0xFF) << 32;
    n |= (uint64_t) (a[2] & 0xFF) << 40;
    n |= (uint64_t) (a[1] & 0xFF) << 48;
    n |= (uint64_t) (a[0] & 0xFF) << 56;
    return n;
}
...
char netherchecker(int64_t seed, int* fortressQuadrant){
    ...
    uint64_t modulus = 1ULL << 48;
    uint64_t AA = 341873128712;
    uint64_t BB = 132897987541;
    int bastionCount = 0;
    *fortressQuadrant = 0;
...
int strongholdSlowCheck(int64_t seed, int fortressQuadrant, LayerStack* gp){
    ...
    int mc = MC_1_16;
    Pos pos_sh = initFirstStronghold(&sh, mc, seed);
    int64_t sh_dist = 0xffffffffffffLL;
    long temp = 0;
    int i, N = 3;
    Pos best_sh;
    for (i = 1; i <= N; i++)
